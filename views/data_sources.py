...
def _sources_panel() -> None:
    ...
    rows = [
        ("Monthly production — REAL", "Colorado ECMC (COGCC) public records",
         "28 DJ Basin Niobrara/Codell horizontals (Weld Co.) under real state API "
         "ids. Free public data, committed. Monthly cadence — no ESP telemetry, no "
         "injection surveys, no failure labels."),
        ("Monthly production — synthetic", "pec modeled fleet (well_001–041)",
         "Known-ground-truth well files incl. ESP readings and dyno cards for the "
         "diagnostics; committed upstream."),
        ("Fleet SCADA — synthetic", "esp generator (well_001–100, seed 7)",
         "Daily pump telemetry (BFPD, intake, motor temp/amps, runtime, VSD Hz, "
         "current imbalance) + labeled failures + run-life ground truth. "
         "Regenerated by bootstrap; gitignored."),
        ("Injection surveys — synthetic", "gla generator (well_001–020, seed 42)",
         ...
         "parameters. Regenerated by bootstrap; gitignored."),
    ]
...
def _production_upload() -> None:
    ...
    for w in wells:
        ...
        rows.append({"Well": w.well_id, "Months": len(hist),
                     "Latest Oil (BOPD)": hist[-1]["oil_bopd"] if hist else None,
                     "Decline Fit": fit_note})
    st.dataframe(pd.DataFrame(rows), width="stretch", hide_index=True)
    theme.data_badge("real", "Your uploaded production — parsed by pec's monthly "
                             "adapter for this session only.")
...
def _scada_upload() -> None:
    ...
    theme.data_badge("real", "Your uploaded fleet SCADA — scored in-session; the "
                             "registry's synthetic identity is NOT joined onto "
                             "your wells.")
...
def _injection_upload() -> None:
    ...
    st.dataframe(pd.DataFrame(rows), width="stretch", hide_index=True)
    theme.data_badge("real", "Your uploaded injection surveys — GLPC fit + "
                             f"optimum at ${oil_price:,.0f}/bbl, "
                             f"${gas_cost:.2f}/Mscf, {nri:.0%} NRI.")
